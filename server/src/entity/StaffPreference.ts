...
export class StaffPreference extends BaseEntity {
  ...
  static async insertStaffPreferencesIntoDb(valueToInsert: IStaffPreference){
    ...
    try{
      ...
      .where("staff_pref.staffId = :staffId AND staff_pref.unitId = :unitId", { staffId: valueToInsert.staffId, unitId: valueToInsert.unitId })
      .getOneOrFail();
    ...
    if (inDB){    // if already in db, then just update all the values
      await getConnection()
            ...
            .set({ preferenceScore: valueToInsert.preferenceScore, lecturerScore: valueToInsert.lecturerScore, isHeadTutorCandidate: valueToInsert.isHeadTutorCandidate})
            .where("staff_pref.staffId = :staffId AND staff_pref.unitId = :unitId", { staffId: valueToInsert.staffId, unitId: valueToInsert.unitId })
            .execute();
    }
  }
